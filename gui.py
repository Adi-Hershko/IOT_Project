...
class MC(Mqtt_client):
    ...
    def on_message(self, client, userdata, msg):
            ...
            if 'Home' in topic:               
                if WatMet:
                    mainwin.graphsDock.update_electricity_meter(check(m_decode.split('Electricity: ')[1].split(' HeartRate: ')[0]))
                    WatMet = False
                else:
                    mainwin.graphsDock.update_heartrate(check(m_decode.split(' HeartRate: ')[1]))
                    WatMet = True
            if 'alarm' in topic:            
                mainwin.statusDock.update_mess_win(da.timestamp()+': ' + m_decode)
            if 'heartrate' in topic:
                mainwin.statusDock.bodyTemp.setText(check(m_decode.split('Temperature: ')[1]))
...
class StatusDock(QDockWidget):
    ...
    def __init__(self,mc):
        QDockWidget.__init__(self)        
        self.mc = mc
        self.bodyTemp = QLabel()
        self.bodyTemp.setText("80")
        self.bodyTemp.setStyleSheet("color: red")
        self.wifi = QLabel()
...
class GraphsDock(QDockWidget):
    ...
    def __init__(self,mc):
        ...
        self.eHeartRateButton = QPushButton("Show",self)
        self.eHeartRateButton.clicked.connect(self.on_button_HeartRate_click)
        self.eHeartRateText= QLineEdit()
        self.eHeartRateText.setText(" ")
        ...
        formLayot.addRow("HeartRate",self.eWaterButton)
        formLayot.addRow(" ", self.eHeartRateText)
    ...
    def update_heartrate(self, text):
        self.eHeartRateText.setText(text)
    ...
    def on_button_HeartRate_click(self):
       self.update_plot(self.stratDateStr, self.endDateStr, 'HeartRate')
       self.eHeartRateButton.setStyleSheet("background-color: yellow")
...
class TempDock(QDockWidget):
    ...
    def __init__(self,mc):
        ...
        self.tTemp.addItems(["Auto", "ON", "OFF"])
        self.tTemp.currentIndexChanged.connect(self.tb_selectionchange)
        self.tsetButton = QPushButton("SET(UPDATE)",self)
        self.tsetButton.clicked.connect(self.on_tsetButton_click)
        formLayot=QFormLayout()       
        formLayot.addRow("Temp sensor",self.tTemp)
    ...
    def on_tsetButton_click(self):
        ...
        time.sleep(0.2)
        if "ON" in self.tTemp.currentText():
            self.tTemp.setStyleSheet("color: green")
            self.mc.publish_to(comm_topic+'Temp/sub','Set temperature to: ON')

    def tb_selectionchange(self,i):
        print ("Current index",i,"selection changed ",self.tTemp.currentText())
        if "ON" in self.tTemp.currentText():
            self.tTemp.setStyleSheet("color: green")
        elif "OFF" in self.tTemp.currentText():
            self.tTemp.setStyleSheet("color: red")
        else:
            self.tTemp.setStyleSheet("color: none")
