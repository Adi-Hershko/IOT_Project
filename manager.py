# intended to manage the BFit system
# insert data to BFit DB
...
def insert_DB(topic, m_decode):
    ...
    # Elec Meter case:
        ...
        da.add_IOT_data('ElectricityMeter', da.timestamp(), m_decode.split(' Electricity: ')[1].split(' HeartRate: ')[0])
        da.add_IOT_data('HeartRate', da.timestamp(), m_decode.split(' HeartRate: ')[1])
...
def check_DB_for_change(client):
    ...
    df = da.fetch_data(db_name, 'data', 'HeartRate')
    
    if len(df.value)==0: return

    if float(df.value[len(df.value)-1]) > heartbitMax:
        msg = 'Current heartbit exceed the normal! '+df.value[len(df.value)-1]
        ic(msg)
