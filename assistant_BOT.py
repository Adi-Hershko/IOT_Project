        ...
        while True:
            ...
            if "home status" in userresponcestring:
                ...
                if "yes" in userresponcestring:                    
                    dfW = da.fetch_data(db_name, 'data', 'HeartRate').value
                    if len(dfW)==0:
                    ...
                    text_msg = 'The current home state: electricity average consumption is '+ E_report +' kiloWatt per hour and operated under normal condition, HeartRate average is '+ W_report
            ...
            if "room temperature" in userresponcestring:
                ...
                if 'yes' in userresponcestring:
                    ...
                    icA('At which heartrate would you like to set the alarm?')
                    if path.exists("what_heartrate.wav"):
                        pl.play("what_heartrate.wav")
                    else:    
                        ts.save2file(ts.tts_request('At which heartrate would you like to set the alarm?'),ttsfile)
                        pl.play(ttsfile)
                    time.sleep(sys_delay)
            ...
            # here should be analitics request to manager
                ...
                if 'weekly' in userresponcestring:
                    ...
                    time.sleep(sys_delay)
                    continue
            if "Blood Pressure" in userresponcestring:
            # here should be analitics request to manager
                icA('Data request..')
                ts.save2file(ts.tts_request('The blood pressure 240/80 , would you like to turn on the alarm ?'),ttsfile)
                time.sleep(sys_delay)
                pl.play(ttsfile)
                time.sleep(sys_delay)
                pl.record(userresponcefile)
