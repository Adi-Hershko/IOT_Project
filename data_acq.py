...
def init_db(database):
    # database = r"data\BFitData.db"    
    # 
    #
    # - here we deleted addres an 
    # + we added BFitSerialNumber 
    tables = [
...
def csv_acq_data(table_name):
        ...
        try:
            if db_init:                
                data = pd.read_csv("data/BFitData.csv")
...
def create_IOT_dev(name, status, units, last_updated, update_interval, BFitSerialNumber, placed, dev_type, enabled, state, mode, fan, temperature, dev_pub_topic, dev_sub_topic, special):
    """
    ...
    :return: sys_id
    """
    sql = ''' INSERT INTO iot_devices(name, status, units, last_updated, update_interval, BFitSerialNumber, placed, dev_type, enabled, state, mode, fan, temperature, dev_pub_topic, dev_sub_topic, special)
              VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) '''
    conn = create_connection()
    if conn is not None:
        cur = conn.cursor()
        cur.execute(sql, [name, status, units, last_updated, update_interval, BFitSerialNumber, placed, dev_type, enabled, state, mode, fan, temperature, dev_pub_topic, dev_sub_topic, special])
        conn.commit()
        re = cur.lastrowid
        conn.close()
        return re
...
if __name__ == '__main__':
    if db_init:
        ...
        numb =create_IOT_dev('alarm', 'off', 'N', timestamp(), 300, 'BFitSerialNumber', 'left front', 'alarm', 'false', 'cooling', 'mode', 'fan', '32', comm_topic+'ala-1/pub', comm_topic+'ala-1/sub', 'changed')
        numb =create_IOT_dev('DHT-1', 'on', 'celcius', timestamp(), 300, 'BFitSerialNumber', 'placed', 'detector', 'enabled', 'state', 'mode', 'fan', 'temperature', comm_topic+'DHT-1/pub', comm_topic+'DHT-1/sub', 'done')
        numb =create_IOT_dev('DHT-2', 'on', 'celcius', timestamp(), 300, 'BFitSerialNumber', 'placed', 'detector', 'enabled', 'state', 'mode', 'fan', 'temperature', comm_topic+'DHT-2/pub', comm_topic+'DHT-2/sub', 'done')
        numb =create_IOT_dev('HeartRate', 'on', 'N', timestamp(), 3600, 'BFitSerialNumber', 'placed', 'meter', 'enabled', 'state', 'mode', 'fan', 'NA', comm_topic+'HeartRate/pub', comm_topic+'HeartRate/sub', 'done')
        ...
        for d in range(15,30):
            ...
            for h in range(0,23):
                ...
                add_IOT_data('HeartRate', '2021-05-'+ str(d+1) + ' ' + str(h) + ':30:00', current_w)

    
    rez= filter_by_date('data','2021-05-16','2021-05-18', 'HeartRate')
    print(rez)
